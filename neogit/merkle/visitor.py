...
from neogit.core.model import FSDirectoryNode, FSFileNode, MerkleNode, Node
...
class NeoMerkleTreeBuilder(BetterContextManager):
    ...
    console: AbstractConsoleAdapter = field(default=DEFAULT_ADAPTER)
    ...
    def safe_enter(self):
        ...
        self.uploader_thread = self.ex.enter_context(
            ObjectUploaderThread(self.ts_object, self.uploader_queue, console=self.console)
        )
        return self
    ...
    def run(self):
        ...
        for item in self.visitor.as_gen():
            ...
            if isinstance(item.node, FSFileNode):
                self.console.on_file_hashed(item.node.path)
                continue
            if isinstance(item.node, FSDirectoryNode):
                self.console.on_dir_merkelized(item.node.path)
                # directory, upload it to Neo4j
                Tree.create_from_merkle_node_cypher(self.session, item.return_value)
                self.console.on_tree_merged()
